...
describe("Only One to One Related Control Element Test Case", function () {
  ...
  it.only("Create Related New for One to One Related Control", function () {
    ...
    cy.contains(this.RelatedKitItemData.CheckboxSelectValue5).click({
      force: true,
    });
  ...
  it("Link Existing Item for OneToOne Related Control", function () {
    cy.wait(2000);
  ...
  it("Validate Link Item elements for OneToOne", function () {
    //Scroll
    cy.get(".last-updated:nth-child(2) > .v-icon").scrollIntoView({
      force: true,
    });
